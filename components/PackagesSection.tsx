...
const PackagesSection: React.FC<PackagesSectionProps> = ({ onCtaClick }) => {
  ...
  const packages = [
    ...
    },
    {
      id: 2,
      name: 'Full Car',
      image: 'https://images.unsplash.com/photo-1567818735868-e71b99932e29?auto=format&fit=crop&q=80&w=1920',
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-10">
          {packages.map((pkg, idx) => (
            <button
              ...
              className={`px-5 py-3 rounded-full text-[10px] md:text-xs transition-all duration-500 border font-unbounded uppercase tracking-widest ${
                activePackage === idx
          ...
          <div className="relative bg-white rounded-[2.5rem] border border-gray-100 overflow-hidden shadow-xl">
            ...
            <div className="p-6 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <div className="w-full md:w-auto flex-shrink-0">
                <button
                  onClick={onCtaClick}
                  className="group/btn relative px-10 py-4 bg-brand text-black text-sm md:text-base rounded-full transition-all duration-500 shadow-md overflow-hidden active:scale-[0.98] font-unbounded"
                >
                  <span className="relative z-10 font-unbounded uppercase tracking-[0.1em]">
